...
export default function Website({ generate }) {
  ...
  return (
    <>
      ...
      <div className={styles.formGroup}>
        <label for="inputUrlSearch">Adres URL do wyszukiwania w witrynie wewnętrznej (przed query)</label>
        <input type="url" id="inputUrlSearch" value={inputUrlSearch} onChange={e => setInputUrlSearch(e.target.value)}></input>
      </div>
      <div className={styles.grid}>
        <div className={styles.formGroup}>
          <label for="inputStringInSearch">Opcjonalnie: ciąg znaków w wyszukiwanym adresie URL występujący po zapytaniu</label>
